#line 1 "/home/duarte/Arduino/lab_SCDTR_final_1st/com.cpp"
#include "main.hpp"
#include <ACAN2515.h>
...
/*
 * @brief Send the information to the serial port
 */
bool send_info(void *)
{
...
{
  // Use o unique_id[5] para definir o CAN ID
  // Mapeie valores específicos do unique_id[5] para IDs específicos do CAN
  switch (unique_id[5])
  {
  case 0x64:
    return 0x01;
...
bool read_can_msg(void *)
{
  ...
  if (can.available())
  {
    ...
    else // If the last bit is 0, then it is an answer
    {
      ...
      for (uint8_t i = 0; i < message.len; i++) // Print the data
